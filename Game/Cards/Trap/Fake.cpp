...
namespace Card{
		...
		void Fake::burnUpdate(){
			particleUnit.swapFlames();
			particleUnit.particleAmtran = glm::vec4(0.5f,0.1f,0.9f,1.0f);
			chain = ZYUG_FADECARD;
			wait(0.5f);
		}
		void Fake::fadeCardUpdate(){
			attackCard->bigRender.amtran = glm::vec4(1.0f,1.0f,1.0f,1.0f);
			attackCard->bigRender.interpolateAmtran(glm::vec4(1.0f,1.0f,1.0f,0.0f),0.3f);
